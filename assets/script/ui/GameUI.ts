import { _decorator, Component, Node, Label, Sprite, SpriteFrame, loader, Animation, resources, assetManager, ImageAsset, Texture2D, Vec3 } from "cc";
...
export class GameUI extends Component {
    ...
    private _talking(customerID: string){
        const table: any = Constants.talkTable;
        const index = Math.floor(Math.random() * table.length);
        const str = table[index].name + ' 是 ' + table[index].collect;
        this.content.string = str;
        this.talkNode.active = true;
        const url = table[index].url;
        assetManager.loadRemote<ImageAsset>(url, (err: any, imageAsset: any)=>{
            if(err){
                return;
            }
